...
// Helper Functions
const calculateMoodFrequencyByCategory = (records: MoodRecord[]): MoodCategory[] => {
...
const parseTimestamp = (timestamp: string): Date => {
  ...
  const cleanTimestamp = timestamp.replace(/(st|nd|rd|th)/, "").replace(",", "");
...
const getFilteredMoodRecords = (
  ...
  switch (filter) {
    case "week":
      return filterRecordsByDateRange(moodRecords, new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000), now);
    case "month":
      return filterRecordsByDateRange(moodRecords, new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000), now);
    case "custom":
      if (customStartDate && customEndDate) {
        return filterRecordsByDateRange(moodRecords, new Date(customStartDate), new Date(customEndDate));
      }
      return moodRecords;
    case "all":
    default:
...
const MoodSummary = ({ moodRecords }: { moodRecords: MoodRecord[] }) => {
  ...
  const filteredMoodRecords = getFilteredMoodRecords(moodRecords, filter, customStartDate, customEndDate);
  ...
  return (
    <div className="h-full w-full flex flex-col items-center p-3">
      <h1 className="font-semibold text-2xl text-gray-700 mb-4 text-center">Mood Summary</h1>

      <div className="flex justify-center space-x-4 mb-4 items-center">
        <button
          onClick={() => setFilter("week")}
          className={`p-2 ${filter === "week" ? "bg-blue-500 text-white" : "bg-gray-200"} rounded-lg`}
        >
          Last Week
        </button>
        <button
          onClick={() => setFilter("month")}
          className={`p-2 ${filter === "month" ? "bg-blue-500 text-white" : "bg-gray-200"} rounded-lg`}
        >
          Last Month
        </button>
        <button
          onClick={() => setFilter("all")}
          className={`p-2 ${filter === "all" ? "bg-blue-500 text-white" : "bg-gray-200"} rounded-lg`}
        >
          All Data
        </button>
        <button
          onClick={() => setFilter("custom")}
          className={`p-2 ${filter === "custom" ? "bg-blue-500 text-white" : "bg-gray-200"} rounded-lg`}
        >
          Custom Date
        </button>
      ...
      <div className="flex justify-between w-full max-w-6xl h-full">
        <div className="flex flex-col justify-center items-center w-1/2 p-6 bg-gray-100 rounded-lg">
          <div className="flex flex-wrap gap-4">
            {moodData.map(({ percentage, color }, index) => (
              <div key={index} className="m-2">
                <div
                  className="flex items-center justify-center rounded-full text-white relative text-center shadow-lg"
                  style={{
                    width: `${percentage * 4}px`,
                    height: `${percentage * 4}px`,
                    fontSize: `${percentage / 2}px`,
        ...
        <div className="flex-1 bg-gray-100 p-6 ml-8 rounded-lg shadow-lg">
          <ul>
            {moodData.map(({ category, count, color }, index) => (
